...
const CryptoDetailsCard = ({ statsArray, title }: Props) => (
  <div className='w-full flex flex-col gap-[16px] basis-[50%] bg-white p-[16px] rounded-[24px] shadow-light'>
    <h1 className='text-center sm:text-left'>{title}</h1>
    <div className='flex flex-col gap-[16px]'>
      {statsArray.map(({ icon, title, value }, i) => (
        <div key={i} className='flex justify-between w-full'>
          <div className='flex items-center gap-[8px] text-lighterOceanBlue text-2xl'>
            {icon}
